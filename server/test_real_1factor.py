...
# Si le script est dans 'server', son dossier parent est la racine du projet
...
# 3. Maintenant, l'importation se fera sans aucune erreur !
import gestion.multifactor.markowitz_1factor as markowitz_1factor

def run_local_simulation():
    print("=" * 60)
    print("TEST DU VRAI MODÈLE 1 FACTEUR AVEC GRADIENT OPTIMAL")
    print("=" * 60)
    ...
    print("Calcul des bêtas et de la matrice de covariance en cours...")

    # Ajoute ceci temporairement dans ton test_real_1factor.py pour auditer les données :
    print("Vérification des inputs de ton modèle :")
    ...
    # Appel de la fonction de ton fichier original
    # On force la méthode 'gradient_optimal'
    result = markowitz_1factor.run(
        tickers=tickers, 
        start=start_date, 
        end=end_date, 
    ...
    if "error" in result:
        ...
        print("\n[SUCCÈS] Simulation terminée avec succès ! ")
        ...
        print("\nAllocations d'actifs calculées par le Gradient Optimal :")
        print("-" * 55)
